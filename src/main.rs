...
fn try_autocorrect_script(manager: &str, cmd: &str, dir: &Path) -> String {
    ...
    let known_commands = match manager {
        ...
        "yarn" => vec![
            "install", "i", "add", "a", "remove", "rm", "start", "s", "test", "t", "upgrade", "up",
            "list", "ls", "init", "publish", "pack", "version", "audit",
        ],
        "pnpm" => vec![
            "install", "i", "add", "a", "remove", "rm", "start", "s", "test", "t", "update", "up",
            "list", "ls", "init", "publish", "pack", "version", "audit",
        ],
        "bun" => vec![
            "install", "i", "add", "a", "remove", "rm", "start", "s", "test", "t", "update", "up",
...
fn patch_npm_command(cmd: &str) -> Vec<String> {
    match cmd {
        ...
        _ => {
            // npm built-in commands that don't need 'run' prefix
            let npm_builtins = [
                "install",
                "uninstall",
                "start",
            ...
            if npm_builtins.contains(&cmd) {
                vec![cmd.to_string()]
            } else {
                // Custom script - needs 'run' prefix for npm
                vec!["run".to_string(), cmd.to_string()]
            }
        }
    }
}
    ...
    if let Ok(scripts) = read_package_json_scripts(dir) {
        if !scripts.is_empty() {
            ...
            if let Some((suggested, _score)) = find_similar_command(cmd, &scripts) {
                ...
                eprintln!(
                    "{}",
                    style(format!("✓ Autocorrected '{}' → '{}'", cmd, suggested))
                        .yellow()
                        .bold()
                );
                return suggested;
...
mod tests {
    ...
    fn test_patch_npm_vs_yarn_add_command() {
        ...
        let npm_result = patch_commands(
            "npm",
            &vec!["a".to_string(), "lodash".to_string()],
            temp_dir.path(),
        );
        assert_eq!(npm_result, vec!["install", "lodash"]);

        // yarn has 'add', should map 'a' to 'add'
        let yarn_result = patch_commands(
            "yarn",
            &vec!["a".to_string(), "lodash".to_string()],
            temp_dir.path(),
        );
    ...
    fn test_try_autocorrect_script_known_commands() {
        ...
        assert_eq!(
            try_autocorrect_script("npm", "install", temp_dir.path()),
            "install"
        );
        assert_eq!(try_autocorrect_script("npm", "i", temp_dir.path()), "i");
        assert_eq!(
            try_autocorrect_script("yarn", "add", temp_dir.path()),
            "add"
        );
        assert_eq!(
            try_autocorrect_script("pnpm", "remove", temp_dir.path()),
            "remove"
        );
    }
    ...
    fn test_patch_commands_with_autocorrect() {
        ...
        // Test that custom script gets 'run' prefix for npm
        let args = vec!["typecheck".to_string()];
        let result = patch_commands("npm", &args, temp_dir.path());
        assert_eq!(result, vec!["run", "typecheck"]);
    }

    #[test]
    fn test_npm_custom_script_gets_run_prefix() {
        let temp_dir = TempDir::new().unwrap();

        // Custom scripts should get 'run' prefix for npm
        let result = patch_commands("npm", &vec!["lint".to_string()], temp_dir.path());
        assert_eq!(result, vec!["run", "lint"]);

        let result = patch_commands("npm", &vec!["typecheck".to_string()], temp_dir.path());
        assert_eq!(result, vec!["run", "typecheck"]);

        let result = patch_commands(
            "npm",
            &vec!["my-custom-script".to_string()],
            temp_dir.path(),
        );
        assert_eq!(result, vec!["run", "my-custom-script"]);
    }

    #[test]
    fn test_npm_builtin_commands_no_run_prefix() {
        let temp_dir = TempDir::new().unwrap();

        // npm built-in commands should NOT get 'run' prefix
        let result = patch_commands("npm", &vec!["install".to_string()], temp_dir.path());
        assert_eq!(result, vec!["install"]);

        let result = patch_commands("npm", &vec!["publish".to_string()], temp_dir.path());
        assert_eq!(result, vec!["publish"]);

        let result = patch_commands("npm", &vec!["audit".to_string()], temp_dir.path());
        assert_eq!(result, vec!["audit"]);

        let result = patch_commands("npm", &vec!["outdated".to_string()], temp_dir.path());
        assert_eq!(result, vec!["outdated"]);
    }

    #[test]
    fn test_npm_explicit_run_not_doubled() {
        let temp_dir = TempDir::new().unwrap();

        // If user explicitly types 'run', it should not become 'run run'
        let result = patch_commands(
            "npm",
